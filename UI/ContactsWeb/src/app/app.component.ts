...
export class AppComponent {
  ...
  onFormSubmit() {
    ...
    this.http.post('https://localhost:7101/api/Contacts', addContactRequest)
    .subscribe({
      next: (value) => {
        console.log(value);
  ...
  onDelete(id: string) {
    this.http.delete(`https://localhost:7101/api/Contacts/${id}`)
    .subscribe({
      next: (value) => {
        alert('Item deleted');
        this.contacts$ = this.getContacts();
...
private getContacts(): Observable<Contact[]> {
  return this.http.get<Contact[]>('https://localhost:7101/api/Contacts');
}
}
